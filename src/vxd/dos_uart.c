...
static void vm_dos_uart_recompute_irq(VM_DOS_UART_STATE *uart)
{
    ...
    if ((uart->ier & VM_DOS_UART_IER_RX_DATA) != 0U &&
        (uart->lsr & VM_DOS_UART_LSR_DR) != 0U) {
                               ...
                               VM_DOS_UART_EVENT *event)
{
    ...
    case VM_DOS_UART_REG_LSR:
        value = uart->lsr;
        uart->lsr &= (unsigned char)(~VM_DOS_UART_LSR_OE);
        break;
...
unsigned char vm_dos_uart_get_iir(const VM_DOS_UART_STATE *uart)
{
    ...
    if (uart->pending_irq == VMODEM_DOS_UART_IRQ_MODEM_STATUS) {
